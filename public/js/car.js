var carz = (function(){
	...
	EVENTS.call = {
		...
		openGetDirections : function($){
			$(document).on("click","input#wdgt-z-car-submit-button, div#wdgt-z-car-driving-get-directions",function(){
				var url,from,to;
				from = $("input#wdgt-z-car-from-input").val();
				to = $("input#wdgt-z-car-to-input").val();
				url = absolute_url+"templates/gmaps.html?from="+from+"&to="+to;
		...
		toFieldFocus : function($){
			...
			$("#wdgt-z-car-to-input").focus(function(){
				address = $(this).val();
				$(this).val('');
	...
	GOOGLEMAPS.call = {
		getTravelTime : function($){
			...
			origin = $("input#wdgt-z-car-from-input").val();
			destination = $("input#wdgt-z-car-to-input").val();
	...
	DYNAMICELEMENTS.call = {
		...
		handdler : function($){
			...
			$("#wdgt-z-car-to-input").on("change",function(){
